#!/usr/bin/env node
/**
A summary of mappings from LM -> profiles
 * */
...
let lmId = process.argv[2]; 

lmId = "ActNowComplete"     //temp

if (!lmId) {
    console.log("No LogicalModel specified. Must be in the command eg: ./makeProfilesAndExtensions ActNowReview")
    return;
}

let fullPath = "fsh-generated/resources/StructureDefinition-"+ lmId + ".json";
let pathToSummary = "input/pagecontent/"+lmId+"mapping.xml"
...
ar.push("<!-- Generated by the makeLMMapping script -->")

ar.push("<br/><strong>Mappings for Logical Model "+lmId+"</strong><br/><br/>")
ar.push("<table class='table table-bordered table-hover table-sm'>")
ar.push("<tr><th>Path</th><th>Mapping</th><th>Comments</th></tr>")
...
SD.snapshot.element.forEach(function(ed){
    ...
    if (ed.mapping && ed.mapping.length > 0) {
        ar.push(`<td>${ed.mapping[0].map}</td>`)
        ar.push(`<td>${ed.mapping[0].comment}</td>`)
    } else {
        ar.push(`<td></td>`)
        ar.push(`<td></td>`)
    }
...
let summaryContents = ar.join('\r\n')
fs.writeFileSync(pathToSummary,summaryContents)
